...
function sessionPayload({ sessionId, round, panel, project, students, examiners }) {
  const chair = examiners[0];
  const startedAt = new Date('2026-09-14T09:00:00.000Z');
  const vivaEndsAt = new Date('2026-09-14T09:30:00.000Z');
  const extraGradingEndsAt = new Date('2026-09-14T09:40:00.000Z');
  ...
  return {
    ...
    projectSnapshot: {
      ...
      supervisor: personSnapshot(chair),
    },
    panelSnapshot: {
      panelId: String(panel._id),
      chair: personSnapshot(chair),
      examiners: examiners.map(personSnapshot),
    },
    examinerSheets: examiners.map((examiner, index) => ({
      examiner: personSnapshot(examiner),
      scores: index === 0
        ? [{ factorId: 'presentation', value: 8, source: 'examiner', recordedAt: startedAt }]
        : [],
    })),
  };
}
...
export async function runVivaPersistenceIntegration(testDatabaseUri) {
  ...
  await assert.rejects(
    () => VivaRound.create({ ...roundConfiguration, factors: [] }),
  ...
  const initialSessionId = await createSessionWithAudit(
    sessionPayload({
      ...
      examiners: [examinerOne, examinerTwo],
    }),
    admin
  );
  ...
  assert.equal(historicalSession.panelSnapshot.chair.name, 'Examiner One');
  ...
  await VivaSession.updateOne(
    ...
    { $set: { cancelledAt: new Date('2026-09-14T09:05:00.000Z'), cancellationReason: 'Room unavailable' } }
  );
  ...
  assert.notEqual(replacementSessionId, initialSessionId);
  ...
  assert.equal(
    ...
    console.log(JSON.stringify({
      ...
      seededUsers: 7,
      verified: ['schema-validation', 'unique-panel', 'history-snapshots', 'snapshot-file-reference', 'replacement-attempt', 'rollback', 'concurrent-write'],
    }));
  } finally {
