...
- Long local validation: `npm run test:unit` exited 1 in the sandbox with 55/56 passing because `tests/s3-client.test.mjs` cannot bind its loopback server there; `node --test tests/s3-client.test.mjs` exited 0 with host networking, so all 56 test files pass with the required host access. `npm run lint` exited 0 with five existing warnings in ignored one-off scripts, `npx tsc --noEmit` exited 0, `npm run build` exited 0, and the disposable Go test suite exited 0.
